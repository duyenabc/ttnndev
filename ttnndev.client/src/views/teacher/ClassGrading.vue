  ...
  <div class="ims-scope">
    ...
    <div class="bg-white rounded-2xl border border-outline-variant overflow-hidden">
      <div class="flex flex-wrap items-center gap-3 p-4 border-b border-outline-variant">
        ...
        <button class="px-4 py-2 rounded-lg font-medium text-body-md border border-outline-variant flex items-center gap-1 disabled:opacity-40"
                :disabled="locked"
                @click="showAddCol = true">
          <span class="material-symbols-outlined text-[18px]">add</span> Thêm cột điểm
        </button>
        <button class="text-white py-2 px-4 rounded-lg font-medium text-body-md flex items-center gap-1 shadow-sm disabled:opacity-40"
        ...
        <table class="w-full text-body-md">
          ...
          <tbody>
            ...
            <tr v-else-if="!rows.length">
              <td :colspan="columns.length + 2" class="px-4 py-12 text-center text-on-surface-variant">
            ...
            <tr v-for="r in rows" :key="r.maGhiDanh" class="border-b border-outline-variant/60">
              ...
              <td v-for="c in columns" :key="c.maCotDiem" class="px-4 py-3">
                <input v-model.number="r.diem[c.maCotDiem]"
                       type="number"
                       step="0.1"
                       min="0"
                       :max="c.diemToiDa"
